...
import kr.co.marketbill.marketbillcoreserver.legacy.application.service.cart.BatchProcessor
...
class OrderFetcher {
    ...
    @Autowired
    private lateinit var batchProcessor: kr.co.marketbill.marketbillcoreserver.legacy.application.service.cart.BatchProcessor
    ...
    ): Optional<kr.co.marketbill.marketbillcoreserver.legacy.application.dto.response.OrderSheetsAggregate> {
        val token = jwtProvider.filterOnlyToken(authorization)
        var userId = jwtProvider.parseUserId(token)
        val role = jwtProvider.parseUserRole(token)
