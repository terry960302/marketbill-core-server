...
class BizConnectionEventHandler(
    ...
    @EventListener
    fun handleCreatedEvent(event: kr.co.marketbill.marketbillcoreserver.legacy.application.event.BizConnectionCreatedEvent) = runBlocking {
        messagingService.sendApplyBizConnectionSMS(event.targetPhoneNo, event.retailerName)
    }

    @EventListener
    fun handleUpdatedEvent(event: kr.co.marketbill.marketbillcoreserver.legacy.application.event.BizConnectionUpdatedEvent) = runBlocking {
        when (event.status) {
