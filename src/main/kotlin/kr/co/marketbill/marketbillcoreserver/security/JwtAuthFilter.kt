    ...
    override fun doFilterInternal(
        ...
        try {
            ...
            if (isValidToken) {
                ...
                val refreshToken: Optional<String> = tokenService.getRefreshToken(userId)
                if (refreshToken.isEmpty) throw CustomException("There's no refresh token.")
                val isValidRefreshToken = jwtProvider.validateToken(refreshToken.get())
                if (isValidRefreshToken) throw CustomException("Access token is expired. Need to reissue token.")
