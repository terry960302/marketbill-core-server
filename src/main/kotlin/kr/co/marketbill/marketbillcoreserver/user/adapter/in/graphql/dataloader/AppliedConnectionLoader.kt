...
class AppliedConnectionLoader(private val userService: UserService) :
    ...
    override fun load(
        ...
        val context: BizConnectionKeyContext = environment.keyContexts as BizConnectionKeyContext
        val command = FindConnectionsCommand.from(keys, context.status, context.pageInfo)
        val results = userService.findAppliedConnectionsByRetailerIds(command)
