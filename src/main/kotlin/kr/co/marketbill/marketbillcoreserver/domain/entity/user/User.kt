...
data class User(
         ...
         * ```
         */
        @Column(name = "belongs_to", nullable = true) val belongsTo: String? = null,
        ...
        @OneToOne(mappedBy = "retailer") val retailerShoppingSession: ShoppingSession? = null,
        @OneToMany(mappedBy = "wholesaler")
        val wholesalerShoppingSessions: List<ShoppingSession> = listOf(),
        @Transient var applyStatus: ApplyStatus? = null,
        @Transient var bizConnectionId: Long? = null,
        @Transient var connectedEmployer: User? = null,
        @Transient var connectedEmployees: List<User> = listOf(),
) : BaseTime() {
